...
const renderLayoutListItems = (items: string[], warning = false) => items
  .map((item) => {
    const { label, body } = parseLabeledText(item)
...
const transformLayoutSummaryMarkdown = (source: string) => {
  ...
  const ratioMatched = recommendationLine.match(/(\d+(?:\.\d+)?)%/)
  if (!recommendationLine || !ratioMatched || (!advantageLines.length && !riskLines.length && !alternativeLines.length)) {
    return source
  }
  ...
  const recommendationHeader = recommendationLine.split(/综合利用率[:：]?\s*\d+(?:\.\d+)?%/i)[0] || recommendationLine
  const schemeMatched = recommendationHeader.match(/^([A-Z])(?:\s*[：: ]\s*)?/i)
  const schemeCode = schemeMatched?.[1]?.toUpperCase() || ''
  const schemeTitle = recommendationHeader
    .replace(/^[A-Z]\s*[：: ]\s*/i, '')
    .replace(/[，,。；\s]+$/g, '')
    .trim() || '综合表现最优'
  const recommendationDetail = recommendationLine
    .replace(recommendationHeader, '')
    .replace(/综合利用率[:：]?\s*\d+(?:\.\d+)?%\s*[，,。；]?/i, '')
    .replace(/^[，,。；\s]+/, '')
    .trim()

  const noteHtml = introLines.length
    ? `<div class="layout-analysis__note">${ highlightBusinessText(introLines.join(' ')) }</div>`
    : ''
...
export const renderMarkdownText = (content: string) => {
  ...
  const layoutSummaryTransformed = transformLayoutSummaryMarkdown(thinkTransformed)
